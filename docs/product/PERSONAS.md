...
```mermaid
graph LR
    Landing[Landing Page] --> Hero[Hero Section]
    Hero --> Opt[Optimalizér Messaging]
    ...
    Seek --> SeekCTA["Začít zdarma"]
    Seek --> SeekProof[Stories + Safety]
    
    OptHead --> Conversion[Email Registration]
    SeekHead --> Conversion
```
