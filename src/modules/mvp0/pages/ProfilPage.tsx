...
import { useSafetyFlags, useUpdateSafetyFlags } from '../api/exercises';
...
// ============================================================
// HealthQuestionnaire — Safety flags inline component
// ============================================================

const SAFETY_ITEMS = [
  { key: 'epilepsy' as const,       label: 'Epilepsie', warning: 'Zádrže dechu mohou být nebezpečné při epilepsii.' },
  { key: 'pregnancy' as const,      label: 'Těhotenství', warning: 'Intenzivní dechová cvičení se nedoporučují během těhotenství.' },
  { key: 'cardiovascular' as const, label: 'Kardiovaskulární onemocnění', warning: 'Zádrže dechu zatěžují srdce.' },
  { key: 'asthma' as const,         label: 'Astma', warning: 'Omezíme intenzitu cvičení pro tvou bezpečnost.' },
];

function HealthQuestionnaire() {
  const { data: safetyFlags, isLoading } = useSafetyFlags();
  const updateSafetyFlags = useUpdateSafetyFlags();
  const [confirmItem, setConfirmItem] = useState<{ key: keyof typeof SAFETY_ITEMS[0], label: string } | null>(null);
  const [expanded, setExpanded] = useState(false);

  if (isLoading) {
    return (
      <div className="profil-health-questionnaire">
        <div className="profil-health-questionnaire__loading">Načítám zdravotní dotazník...</div>
      </div>
    );
  }

  const completed = safetyFlags?.questionnaire_completed ?? false;
  const anyFlag = safetyFlags
    ? SAFETY_ITEMS.some((item) => safetyFlags[item.key])
    : false;

  function handleCheck(key: 'epilepsy' | 'pregnancy' | 'cardiovascular' | 'asthma', checked: boolean) {
    if (!checked && safetyFlags?.[key]) {
      // Unchecking existing flag — require confirmation
      const item = SAFETY_ITEMS.find((i) => i.key === key);
      if (item) {
        setConfirmItem({ key: item.key as unknown as keyof typeof SAFETY_ITEMS[0], label: item.label });
      }
      return;
    }

    // Checking or unchecking non-flagged item → immediate save
    const newFlags = {
      epilepsy: safetyFlags?.epilepsy ?? false,
      pregnancy: safetyFlags?.pregnancy ?? false,
      cardiovascular: safetyFlags?.cardiovascular ?? false,
      asthma: safetyFlags?.asthma ?? false,
      [key]: checked,
    };
    updateSafetyFlags.mutate(newFlags);
  }

  function handleConfirmUncheck() {
    if (!confirmItem) return;
    const key = confirmItem.key as 'epilepsy' | 'pregnancy' | 'cardiovascular' | 'asthma';
    const newFlags = {
      epilepsy: safetyFlags?.epilepsy ?? false,
      pregnancy: safetyFlags?.pregnancy ?? false,
      cardiovascular: safetyFlags?.cardiovascular ?? false,
  ...
  return (
    <>
      <div className="profil-health-questionnaire">
        <button
          className="profil-health-questionnaire__header"
        ...
        >
          ...
          <div className="profil-kp-questionnaire__info">
            ...
            <p className="profil-kp-questionnaire__status">
              {completed ? (anyFlag ? 'Omezení aktivní' : 'Vyplněno — bez omezení') : 'Nevyplněno'}
            </p>
          </div>
          <svg
        ...
        {expanded && (
          <div className="profil-health-questionnaire__body">
            <p className="profil-health-questionnaire__desc">
              Zaškrtni zdravotní stavy, které se tě týkají. SMART CVIČENÍ se automaticky přizpůsobí.
            </p>
            <div className="profil-health-questionnaire__items">
              {SAFETY_ITEMS.map((item) => {
                const checked = safetyFlags?.[item.key] ?? false;
                return (
                  <label key={item.key} className="profil-health-questionnaire__item">
                    <input
                      type="checkbox"
                      checked={checked}
                      onChange={(e) => handleCheck(item.key, e.target.checked)}
                      className="profil-health-questionnaire__checkbox"
                      aria-label={item.label}
                    />
                    <div>
                      <span className="profil-health-questionnaire__label">{item.label}</span>
                      {checked && (
                        <span className="profil-health-questionnaire__warning">{item.warning}</span>
                      )}
                    </div>
                  </label>
                );
      ...
      {/* Confirm dialog when unchecking an active flag */}
      <ConfirmModal
        isOpen={confirmItem !== null}
        onClose={() => setConfirmItem(null)}
        onConfirm={handleConfirmUncheck}
        title="Odebrat zdravotní omezení?"
        message={`Potvrzuješ, že ${confirmItem?.label.toLowerCase()} již nemáš a toto omezení chceš odebrat?`}
